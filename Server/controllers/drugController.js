...
exports.addDrug = (req, res) => {
  const {
    ...
    db.query(
        'INSERT INTO medicine_stocks (supplier_id, manifacturer, item_measure, code, name, batchNo, quantity, price, sell, expiry) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
        [sellerId, manifacturer, item_measure, code[0], name, batchNo, quantity, price, sell, expiry],
        (err, results) => {
            if (err) {
...
exports.addSeller = (req, res) => {
  ...
  const query = `
    INSERT INTO suppliers (name, phone, address, ttno, invoiceNumber, date)
    VALUES (?, ?, ?, ?, ?, ?)
  `;
...
exports.getDrugs = (req, res) => {
  const query = 'SELECT * FROM medicine_shelf WHERE quantity > 0';
...
exports.placeOrder = (req, res) => {
  ...
  const query = 'INSERT INTO orders (customerId, drugId, item_measure, code, name, batchNo, quantity, price, subtotal, total) VALUES ?';
...
exports.approvedOrders = (req, res) => {
  ...
  const insertApprovalQuery = `
      ...
      (customerId,drugId, item_measure, fname, lname, code, name, batchNo, quantity, price, subtotal, status)
      VALUES ?
  `;
...
exports.finishPayment = (req, res) => {
  ...
  const query =
    'INSERT INTO payments (customerId, drugId, userId, fname, lname, item_measure, code, name, batchNo, quantity, unitPrice, subtotal, total, status, paymentMethod) VALUES ?';
  ...
  try {
    for (const order of orders) {
      ...
      if (drugId && quantity > 0) {
        await db.query('UPDATE medicine_shelf SET quantity = quantity - ? WHERE id = ?', [quantity, drugId]);
      }
    }
...
exports.getTotalPaymentForCardForToday = (req, res) => {
  const query = `
    SELECT SUM(amount) AS totalPayment
    FROM payment_for_card
    WHERE DATE(created_at) = CURDATE()
  `;

  db.query(query, (err, results) => {
    if (err) {
      return res.status(500).json({ error: 'Failed to fetch total payment for today' });
    }
    res.status(200).json({ totalPayment: results[0].totalPayment || 0 }); // Ensure 0 if no payments
  });
};
...
exports.getItems = (req, res) => {
  const query = 'SELECT id, item_name, item_code, preparation, dose, route FROM itemNames';
...
exports.addItems = (req, res) => {
  ...
  const { itemName, itemCode ,preparation, dose, route} = req.body;

  const query = `
    INSERT INTO itemNames (item_name, item_code, preparation, dose, route)
    VALUES (?, ?, ?, ?, ?)
  `;

  db.query(query, [itemName, itemCode, preparation, dose, route], (err, result) => {
    if (err) {
      console.error('Error adding category:', err);
      return res.status(500).json({ error: 'Failed to add category' });
    }
...
exports.sellingReportPerDay = (req, res) => {
  ...
  const query = `
      ...
      JOIN 
          medicine_shelf d ON p.drugId = d.id
      WHERE 
          DATE(p.created_at) = ?`;
...
exports.getAllMedicine = (req, res) => {
  const query = `
      SELECT * 
      FROM medicine_shelf 
      WHERE status = 'approve' AND quantity > 0;
  `;
