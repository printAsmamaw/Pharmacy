...
exports.signin = (req, res) => {
    ...
    db.query('SELECT * FROM users WHERE username = ?', [username], (err, results) => {
        ...
        const token = jwt.sign({ id: user.id, slot_number: user.slot_number, roles: user.role, fname: user.fname, lname: user.lname }, 'secretkey', { expiresIn: '1h' });
...
exports.getEmployeeById = (req, res) => {
    ...
    db.query(query,[id], (err, result) => {
        ...
        // return res.status(200).json({ message: 'Positions fetched successfully', data: results });
    });
};